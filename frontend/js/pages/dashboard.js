class DashboardPage {
    ...
    async render() {
        ...
        m.innerHTML = `<div style="text-align:center;padding:3rem"><div class="spinner"></div><p>Chargement...</p></div>`;
        try {
            const ir = await apiGet('/classes/institutions'); this.institutions = ir.data || [];
    ...
    async afficher(m) {
        ...
        if (!inst) { m.innerHTML = `<div style="text-align:center;padding:4rem"><i class="fas fa-school" style="font-size:3rem;color:var(--text-muted)"></i><h3 style="margin-top:1rem;font-weight:600">Aucune institution</h3><p style="color:var(--text-secondary)">Configurez votre établissement</p></div>`; return; }
  ...
  <div class="dashboard-header">
    ...
    <div class="dashboard-header-center">
      <h2>${inst.nom}</h2>
      <span class="badge badge-info dashboard-institution-badge"><i class="fas fa-${icons[inst.niveau]}"></i> ${labels[inst.niveau]}</span>
    </div>
  ...
  <div class="stats-grid">
    <div class="ui-card"><div class="card-icon-wrapper" style="background:linear-gradient(135deg,#6366f1,#8b5cf6)"><i class="fas fa-user-graduate"></i></div><div class="card-body"><h4 style="font-size:1.75rem;margin:0;font-weight:700">${te}</h4><p style="color:var(--text-secondary);font-size:0.85rem">Élèves inscrits</p></div></div>
    <div class="ui-card"><div class="card-icon-wrapper" style="background:linear-gradient(135deg,#06b6d4,#3b82f6)"><i class="fas fa-chalkboard"></i></div><div class="card-body"><h4 style="font-size:1.75rem;margin:0;font-weight:700">${this.allClasses.length}</h4><p style="color:var(--text-secondary);font-size:0.85rem">Classes</p></div></div>
  </div>
  ${searchRendu}
  <div class="section-header"><h3><i class="fas fa-layer-group"></i> Classes</h3><span style="color:var(--text-muted);font-size:0.8rem">${this.allClasses.length} classe(s)</span></div>
  <div class="classes-grid" id="classes-grid">${this.allClasses.map(c => this.carteClasse(c, inst)).join('')}</div>
  ${this.allClasses.length === 0 ? `<div style="text-align:center;padding:3rem;color:var(--text-muted)"><i class="fas fa-inbox" style="font-size:2.5rem;display:block;margin-bottom:0.75rem"></i><p>Aucune classe trouvée</p></div>` : ''}
</div>`;
    }
    carteClasse(c, inst) {
  ...
  <div class="card-icon-wrapper" style="background:${colors[inst.niveau]}"><i class="fas fa-users"></i></div>
  <div class="card-body"><h4 style="font-weight:600">${c.nom_classe}</h4>
    <div style="display:flex;gap:1rem;margin-top:0.6rem">
      <div><i class="fas fa-user-graduate" style="color:var(--text-muted)"></i> <strong>${c.nb_eleves||0}</strong> <span style="color:var(--text-muted);font-size:0.78rem">élèves</span></div>
      <div><i class="fas fa-user-check" style="color:var(--success)"></i> <strong>${c.presents||0}</strong> <span style="color:var(--text-muted);font-size:0.78rem">présents</span></div>
    </div>
    <div style="margin-top:0.6rem;background:var(--border);border-radius:6px;height:5px"><div style="width:${c.taux||0}%;height:100%;background:var(--success);border-radius:6px;transition:width 0.6s"></div></div>
    <div style="text-align:right;font-size:0.75rem;color:var(--text-muted);margin-top:0.25rem">${c.taux||0}% présence</div>
  </div>
  <div class="card-footer">${aDesOptions?'<span class="badge badge-info"><i class="fas fa-cog"></i> Choisir une option</span>':'<span>Voir la classe <i class="fas fa-arrow-right"></i></span>'}</div></div>`;
    }
    ...
    async ouvrirModalOptions(nomNiveau, classeId) {
        ...
        try {
            ...
            overlay.innerHTML = `<div class="modal" onclick="event.stopPropagation()"><div class="modal-header"><h3><i class="fas fa-cogs"></i> ${nomNiveau} - Choisir une option</h3><button class="modal-close" onclick="document.getElementById('modal-options-overlay').remove()"><i class="fas fa-times"></i></button></div><div class="modal-body" style="display:flex;flex-direction:column;gap:0.6rem">${options.length?options.map(o => `<div class="ui-card card-hoverable" onclick="document.getElementById('modal-options-overlay').remove();router.navigate('classe/${classeId}?option=${o.classe_option_id}')"><div style="display:flex;align-items:center;gap:0.8rem;padding:0.8rem 1rem"><div style="width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#818cf8,#6366f1);display:flex;align-items:center;justify-content:center;color:white;font-size:1rem"><i class="fas fa-${icns[o.code]||'book'}"></i></div><div style="flex:1"><strong style="font-size:0.9rem">${o.nom}</strong><p style="color:var(--text-muted);font-size:0.78rem">Code: ${o.code} · ${o.nb_eleves||0} élèves</p></div><i class="fas fa-chevron-right" style="color:var(--text-muted)"></i></div></div>`).join(''):'<p style="text-align:center;color:var(--text-muted);padding:1rem">Aucune option disponible</p>'}</div><div class="modal-footer"><button class="btn btn-ghost btn-sm" onclick="document.getElementById(\'modal-options-overlay\').remove()"><i class="fas fa-times"></i> Fermer</button></div></div>`;
